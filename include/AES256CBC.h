...
class AES256CBC {
 ...
 public:
   ...
   *  must be inLen % 16 == 0
   *  must be len(iv) == 16
   *  must be len(key) == 32 (256 bit)
   *  must be len(in) == inLen
   *  must be len(out) == len(in)
   * 
  */
  unsigned char *EncryptCBC(const unsigned char in[], unsigned char *out, unsigned int inLen,
   ...
   * 
  */
  unsigned char *DecryptCBC(const unsigned char in[], unsigned char *out, unsigned int inLen,
                            const unsigned char key[], const unsigned char *iv);
