...
void calc_256hash(const char *bytes, size_t bytes_len, unsigned char *hash)
{
	...
	while(c > 0)
	{
		cur_block_size = c>BS?BS:c;
		if(c > BS) c-=BS; else c = 0;
		//c-=BS; c=(c>0?c:0);
		for(size_t li = 0; li < cur_block_size; ++li)
		{
			buff[li] = (unsigned char)bytes[i];
		}
		sha256_update(&ctx, buff, cur_block_size);
	}
